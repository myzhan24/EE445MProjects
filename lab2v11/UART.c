...
void UART_Init(void){
  ...
  NVIC_EN0_R |= NVIC_EN0_INT5;           // enable interrupt 5 in NVIC
	...
	/*
	OS_InitSemaphore(&txFifoEmptyCount, FIFOSIZE);
	OS_InitSemaphore(&txFifoFullCount, 0);
	
	OS_InitSemaphore(&rxFifoEmptyCount, FIFOSIZE);
	OS_InitSemaphore(&rxFifoFullCount, 0);
	*/
