...
void DAS(void){	//will use SW trigger ADC sampling on seq3 with PD3
unsigned long input;  
...
void Printer(unsigned long data){
	...
	UART_OutUDec(data);
	UART_OutCRLF();
	//ST7735_Message(0,2,"The Data: ",data);
...
void ADCTASK(void){
	...
	int i;
	while(1){
	//ADC_Collect(5, FS, &Printer);
	//UART_OutString("ADC Collect Data: ");
	//UART_OutUDec(ADC_In());
	//UART_OutCRLF();
	i = ADC_In();
	ST7735_Message(0,2,"The Data: ",i);
	//UART_OutString("ADC MY Data: ");
	//UART_OutUDec(i);
	//UART_OutCRLF();
		PF2 ^= 0x04;
		//OS_Sleep(300);
...
void print(void){
	...
	while(1){
		...
		i++;
		ST7735_Message(0,1,"The Data: ",i);
		//ST7735_MessageTest("testerino: ",i);
		OS_Sleep(10);
	}
}
...
int adcmain(void){
  ...
  PortF_Init();
	...
	//NumCreated += OS_AddThread(&print,128,1); 
	//NumCreated += OS_AddThread(&print2,128,1); 
	NumCreated += OS_AddThread(&ADCTASK,128,1); 
	//NumCreated += OS_AddThread(&printStuff,128,1); 
	//NumCreated += OS_AddThread(&doNothing,128,1); 
	//NumCreated += OS_AddThread(&print2,128,1); 
...
int lcdmain(void){
	...
	NumCreated += OS_AddThread(&printLCD,128,1); 
