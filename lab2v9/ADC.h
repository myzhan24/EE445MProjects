...
// ADCNum corresponds to ADC0 or ADC1
// sample frequency must be 100 Hz - 10000 Hz
// ADC sampling period: t * (prescale + 1) * (period + 1) seconds
// t * (period + 1)
// 1/80 MHz * (period + 1) seconds
// 0.0000000125 * (period + 1) seconds
// 1/ANS = 100 - 10000 Hz
// ANS = 0.01
// period  = 800000 - 1
// ANS = 0.0001
...
// Changed the current sampling rate of the ADC
// Input: sampleRate 100 Hz - 10000 Hz => period of 8000 - 800000
// Output: none
void ADC_ChangeSampleRate(uint32_t period);


// Changed the input port for ADC collection
// Input: channelNum new input port
// Output: -1 invalid channel given, -2 current channel requested, 0-11 for succesful channel swap
int ADC_Open(uint8_t channelNum);

// collects a sample
// takes the list (maybe linked list) and returns what has been aquisitioned
