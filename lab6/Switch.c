...
void Board_Init(void(*task)(void)){          
  ...
  GPIO_PORTF_CR_R |= (SW1|SW2);  // 2b) enable commit for PF4 
  ...
  GPIO_PORTF_DIR_R &= ~(SW1|SW2);// 5) make PF4 in (built-in buttons)
...
//  delay = SYSCTL_RCGC2_R;        // put a delay here if you are seeing erroneous NMI
  ...
  GPIO_PORTF_DEN_R |= (SW1|SW2); // 7) enable digital I/O on PF4
  ...
  GPIO_PORTF_IS_R &= ~0x01;   // PF0 is edge-sensitive (default setting)
  GPIO_PORTF_IBE_R &= ~0x01;  // PF0 is not both edges (default setting)
  GPIO_PORTF_IEV_R &= ~0x01;   // PF0 falling edge event
  GPIO_PORTF_ICR_R = 0x01;    // clear flag4
  //GPIO_PORTF_IM_R |= 0x01;    // enable interrupt on PF4
	
	EndCritical(status);
}

void Board_Init2(void(*task)(void)){
    long status = StartCritical();
    SW2Task = task;
...
void DisableButton(void) {
	OS_Sleep(2000);	// disables button for [variable]ms. Used to stop unintended button presses
	GPIO_PORTF_IM_R |= 0x10;    // enable interrupt on PF4
	OS_Kill();
}

void DisableButton2(void) {
	OS_Sleep(2000);	// disables button for [variable]ms. Used to stop unintended button presses
	GPIO_PORTF_IM_R |= 0x01;    // enable interrupt on PF4
	OS_Kill();
}
...
void GPIOPortF_Handler(void) {
	if ((GPIO_PORTF_MIS_R&0x00000010) == 0x10) {
	    GPIO_PORTF_ICR_R |= 0x10; //acknowledge flag4
	    GPIO_PORTF_IM_R &= ~0x10;	//disable interrupt on PF4
	    OS_AddThread(&DisableButton,128,1);
	    (*SWTask)();					//execute user task
	}
	if ((GPIO_PORTF_MIS_R&0x00000001) == 0x01) {
	    GPIO_PORTF_ICR_R = 0x01; //acknowledge flag0
	    GPIO_PORTF_IM_R &= ~0x01;	//disable interrupt on PF0
	    OS_AddThread(&DisableButton2,128,5);
