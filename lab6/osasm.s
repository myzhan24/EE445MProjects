...
;start by setting the nodePt to the head of the priority linked list
	LDR		R0, =head			
	LDR		R0,[R0]				;R0 = head
	LDR		R1, =NodePt			;R1 = ADDR of NodePt
	STR		R0,[R1]				;NodePt = head
	LDR		R2,[R0,#4]			;R2 = head->Thread
	LDR		R1, =RunPt			;R1 = ADDR of RunPt
	STR		R2,[R1]				;RunPt = head->Thread
...
sh_CheckHead	PUSH{LR}					;save LR
	...
	BMI		sh_SleepCheckLoop	;  Negative = still sleeping or blocked so load next TCB
