...
#define TIME_1MS    80000 	// assumes 80MHz clock         
...
typedef struct BlockedNode BlockedNodeType;
...
struct TCB{
  uint32_t *StackPt;       // Stack Pointer
	uint32_t kill;
  uint32_t MoreStack[300];  // 396 bytes of stack 
  ...
  uint32_t InitialPSR;     // 0x01000000
	//uint32_t Sleep;					// OS_Sleep will put the thread to a dormant state
	uint32_t id;						//Thread ID, starting from 1
};
...
struct Node{
  ...
  uint32_t TimeSlice; 			// how long to run it
	...
	struct Node *Prev;
  uint32_t Blocked;	// boolean of whether thread is blocked 
	uint32_t id;
  #ifdef DEBUG
  //---------------------User debugging-----------------------
