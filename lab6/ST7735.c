...
void ST7735_PlotPointVF(long data, int min, int max){
OS_bWait(&LCDFree);
	...
	double pixel = 4096.0/scale;

	int i;
   ST7735_PlotBar( (data - min) * pixel ); 
	 ST7735_PlotNext(); // called 128 times
OS_bSignal(&LCDFree);
}

void ST7735_PlotPointVT(long data, int min, int max){
OS_bWait(&LCDFree);
	...
	int i;
   ST7735_PlotPoint( (data - min) * pixel ); 
	 ST7735_PlotNext(); // called 128 times
OS_bSignal(&LCDFree);
}

void ST7735_PlotArrayVT(long* data, int n, int min, int max){
OS_bWait(&LCDFree);
	ST7735_PlotClear(0,4095);  // range from 0 to 4095
	double scale = max - min;
	double pixel = 4096.0/scale;
	...
	for(j = 0; j < 128;j++){
	for(i=0;i<n;i++){
			ST7735_PlotPoint( (data[datai++] - min) * pixel ); 
		}
	//datai+=i;
		ST7735_PlotNext(); 
		// called 128 times
...
void ST7735_PlotArrayVTDub(float* data, int n, int min, int max){
OS_bWait(&LCDFree);
	ST7735_PlotClear(0,4095);  // range from 0 to 4095
	double scale = max - min;
	double pixel = 4096.0/scale;
...
void ST7735_PlotArrayVF(long* data, int n, int min, int max){
OS_bWait(&LCDFree);
	ST7735_PlotClear(0,4095);  // range from 0 to 4095
	double scale = max - min;
	double pixel = 4096.0/scale;
	...
	for(j = 0; j < 128;j++){
	for(i=0;i<n;i++){
			ST7735_PlotBar( ((data[datai++] - min) * pixel) ); 
		}
		ST7735_PlotNext(); 
		// called 128 times
	}
...
int count = 0;
void ST7735_PlotArrayVFDub(double* data, int n, int min, int max){
OS_bWait(&LCDFree);
	count++;
	ST7735_PlotClear(0,4095);  // range from 0 to 4095
	double scale = max - min;
...
//void ST7735_PlotArrayFG(long* gData, int fs, int maxGain)
