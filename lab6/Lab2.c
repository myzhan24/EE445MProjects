...
int testmain2(void){  // Testmain2
  ...
  NumCreated += OS_AddThread(&Thread2b,128,3); 
...
int testmain4(void){   // Testmain4
  ...
  OS_Init();           // initialize, disable interrupts
  NumCreated = 0 ;
  OS_AddPeriodicThread(&BackgroundThread1d,3*PERIOD,0); 
...
int testuartmain(void){
	OS_Init();
	UART_OutUDec(2000001);
	int i;
	while(1){
		return 0;
		}
...
int test3main(void){      // Testmain6  Lab 3
  ...
  OS_Launch(TIME_1MS/2);  // 1ms, doesn't return, interrupts enabled in here
  return 0;             // this never executes
}
